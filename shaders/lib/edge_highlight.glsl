float getEdgeHighlight(vec3 viewPos, vec3 playerPos, float depth, vec3 normal) {
	...
	float depthAddition = 1.0 / (1.0 + playerPosMax * 4.0) * 0.0003 + 0.000001;
	
	screenPos4 = gbufferProjection * vec4(viewPos + tangent, 1.0);
	screenPos = screenPos4.xyz / screenPos4.w * 0.5 + 0.5;
	testDepth = texture2D(DEPTH_BUFFER_WO_TRANS_OR_HANDHELD, screenPos.xy).r;
	testNormal = decodeNormal(texelFetch(OPAQUE_DATA_TEXTURE, ivec2(screenPos.xy * viewSize), 0).zw);
	isHighlight = isHighlight || (testDepth > screenPos.z + depthAddition || (dot(normal, testNormal) < 0.1 && testDepth > screenPos.z + depthAddition * 0.25)) && (clamp(screenPos, 0.0, 1.0) == screenPos);
