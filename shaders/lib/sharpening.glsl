void doSharpening(inout vec3 color  ARGS_OUT) {
	...
	float depthAddition = alteredSharpenDepthAddition * 0.0144 + velocityFactor * 0.042;
	float sharpenAmount = alteredSharpenAmount * 0.37 + sqrt(blockDepth) * depthAddition + velocityFactor * 0.33;
	blur = min(blur, 1.0);
