void doSharpening(inout vec3 color, float depth) {
	...
	float depthAddition = alteredSharpenDepthAddition * 0.022 + velocityFactor * 0.0013;
	float sharpenAmount = alteredSharpenAmount * 0.3 + (sqrt(blockDepth * 0.5 + 1.0) - 1.0) * depthAddition + velocityFactor * 0.066;
