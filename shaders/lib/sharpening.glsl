void doSharpening(inout vec3 color, float blockDepth  ARGS_OUT) {
	...
	#if FXAA_ENABLED == 1
		const float alteredSharpenVelocityAddition = SHARPEN_VEL_ADDITION * 2.0;
		const float alteredSharpenDepthAddition = SHARPEN_DEPTH_ADDITION * 1.0;
	#else
	...
	float depthAddition = alteredSharpenDepthAddition * 0.025 + velocityFactor * 0.08;
	float sharpenAmount = SHARPEN_AMOUNT * 0.45 + sqrt(blockDepth) * depthAddition + velocityFactor * 0.17;
	blur = min(blur, 1.0);
