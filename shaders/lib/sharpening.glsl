void doSharpening(inout vec3 color, float blockDepth  ARGS_OUT) {
	...
	float depthAddition = alteredSharpenDepthAddition * 0.014 + velocityFactor * 0.053;
	float sharpenAmount = alteredSharpenAmount * 0.37 + sqrt(blockDepth) * depthAddition + velocityFactor * 0.28;
	blur = min(blur, 1.0);
