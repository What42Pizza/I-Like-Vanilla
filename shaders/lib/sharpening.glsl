void doSharpening(inout vec3 color  ARGS_OUT) {
	...
	float depthAddition = alteredSharpenDepthAddition * 0.014 + velocityFactor * 0.055;
	float sharpenAmount = alteredSharpenAmount * 0.37 + sqrt(blockDepth) * depthAddition + velocityFactor * 0.32;
	blur = min(blur, 1.0);
