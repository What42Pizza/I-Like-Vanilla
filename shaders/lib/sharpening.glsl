void doSharpening(inout vec3 color, float depth) {
	...
	float depthAddition = alteredSharpenDepthAddition * 0.016 + velocityFactor * 0.0011;
	float sharpenAmount = alteredSharpenAmount * 0.27 + (sqrt(blockDepth * 0.5 + 1.0) - 1.0) * depthAddition + velocityFactor * 0.067;
