void doSharpening(inout vec3 color, float depth  ARGS_OUT) {
	...
	float depthAddition = alteredSharpenDepthAddition * 0.02 + velocityFactor * 0.013;
	float sharpenAmount = alteredSharpenAmount * 0.35 + (sqrt(blockDepth + 1.0) - 1.0) * depthAddition + velocityFactor * 0.35;
