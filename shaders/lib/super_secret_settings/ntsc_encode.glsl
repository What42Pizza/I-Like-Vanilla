...
vec4 getNtscEncoded() {
	vec2 InverseP = vec2(P * pixelSize.x, 0.0);
	...
	vec4 Texel0 = texture2D(MAIN_TEXTURE, C0);
	vec4 Texel1 = texture2D(MAIN_TEXTURE, C1);
	vec4 Texel2 = texture2D(MAIN_TEXTURE, C2);
	vec4 Texel3 = texture2D(MAIN_TEXTURE, C3);

	// Calculate the expected time of the sample.
	vec4 T = A2 * Cy * vec4(viewHeight) + B + Cx;
	...
	return (Encoded - MinC) * InvCRange * vec4(1,1,1,0)+vec4(0,0,0,1);
}
