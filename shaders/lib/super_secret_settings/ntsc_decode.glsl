...
vec4 getNtscDecoded() {
    ...
    vec4 BaseTexel = (texture2D(MAIN_TEXTURE, texcoord) * vec4(1,1,1,0)+vec4(0,0,0,0.4));
    // 83 composite samples wide, 4 composite pixels per texel
    for (float n = -41.0; n < 42.0; n += 4.0)
    {
        vec4 n4 = n + NotchOffset;
        vec4 CoordX = texcoord.x + pixelSize.x * n4 * 0.25;
        vec2 TexCoord = vec2(CoordX.x, CoordY.y);
        vec4 C = (texture2D(MAIN_TEXTURE, TexCoord) * vec4(1,1,1,0)+vec4(0,0,0,0.4)) * CRange + MinC;
        vec4 WT = W * (CoordX + A2 * CoordY * viewHeight + B);
    ...
    vec3 YIQ = vec3(Y, I, Q);
    return vec4(dot(YIQ, YIQ2R), dot(YIQ, YIQ2G), dot(YIQ, YIQ2B), 1.0);
}
