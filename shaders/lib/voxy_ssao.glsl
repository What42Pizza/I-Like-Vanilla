float getVoxyAoAmount(vec3 normal) {
	...
	plusXAmount *= 0.15 + dot(mat3(vxModelViewInv) * xDir, blockPos - 0.5);
	aoAmount = max(aoAmount, plusXAmount);
	...
	minusXAmount *= 0.15 + dot(mat3(vxModelViewInv) * -xDir, blockPos - 0.5);
	aoAmount = max(aoAmount, minusXAmount);
	...
	plusYAmount *= 0.15 + dot(mat3(vxModelViewInv) * yDir, blockPos - 0.5);
	aoAmount = max(aoAmount, plusYAmount);
	...
	minusYAmount *= 0.15 + dot(mat3(vxModelViewInv) * yDir, blockPos - 0.5);
	aoAmount = max(aoAmount, minusYAmount);
