...
pub fn build_ids_for_file(file_name: &str) -> Result<()> {
	...
	let mut entity_mappings = collect_entity_mappings(ids);
	let mut entity_mappings = entity_mappings.iter_mut().map(|(id, blocks)| {blocks.sort(); (id, blocks)}).collect::<Vec<_>>();
	...
	let mut old_entity_mappings = collect_entity_mappings(old_ids);
	let mut old_entity_mappings = old_entity_mappings.iter_mut().map(|(id, blocks)| {blocks.sort(); (id, blocks)}).collect::<Vec<_>>();
